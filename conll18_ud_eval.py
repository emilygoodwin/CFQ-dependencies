...
#deal with casing of outputs
#I am too lazy to retype all thenames of things
CONTENT_DEPRELS = {"NSUBJ"}
for each in CONTENT_DEPREL:
    CONTENT_DEPRELS.add(each.upper())

FUNCTIONAL_DEPRELS = {
    "AUX", "COP", "MARK", "DET", "CLF", "CASE", "CC", "PUNCT"
}
...
def evaluate(gold_ud, system_ud):
    ...
    def alignment_score_sentences(alignment):
        gold = len(alignment.gold_words)
        system = len(alignment.system_words)
        aligned = len(alignment.matched_words)
        ...
        def gold_aligned_system(word):
            return alignment.matched_words_map.get(word, "NotAligned") if word is not None else None
        #a list to store % correctly labeled arcs for each sentence
        sentence_scores = []
        content_label_counts = {}
        for relation in CONTENT_DEPRELS:
            content_label_counts[relation] = [0,0]

        correct_content, correct_function = 0,0
        incorrect_content, incorrect_function = 0,0
        #wordID is a word's position in the sentence
        #sentenceID is the sentence position in the doc
        #total arcs in the sentence 
        sentenceID, total_arcs, wordID = 0, 0, 0

        for words in alignment.matched_words:
            arclabel = words.gold_word.columns[DEPREL]
            #handle sentence boundaries 
            if int(words.gold_word.columns[ID]) < wordID:
                sentence_scores.append(correct_content/(correct_content+incorrect_content))
                wordID, correct_content, correct_function, incorrect_content, incorrect_function, total_arcs = 1, 0, 0, 0, 0, 0
                sentenceID = sentenceID + 1

            if (words.gold_word.columns[HEAD], arclabel) == (words.system_word.columns[HEAD], words.system_word.columns[DEPREL]):
                correct_content = correct_content + words.gold_word.is_content_deprel 
                correct_function = correct_function + words.gold_word.is_functional_deprel
                if words.gold_word.is_content_deprel:
                    content_label_counts[arclabel][0] = content_label_counts[arclabel][0]+1
            else:
                incorrect_function = incorrect_function + words.gold_word.is_functional_deprel
                incorrect_content = incorrect_content + words.gold_word.is_content_deprel
                #Track content scores 
                if words.gold_word.is_content_deprel:
                    content_label_counts[arclabel][1] =content_label_counts[arclabel][1]+1  
        ...
        for item in sentence_scores:
            if item == 1:
                perfect_clas = perfect_clas + 1

        print('percentage of sentences with perfect CLAS:', str(perfect_clas/len(sentence_scores)))
    ...
    #Print alignment for performing sentence-level analyses 
    alignment_score_sentences(alignment)
...
def main():
    ...
    if not args.verbose and not args.counts:
        print("CLAS F1 Score: {:.2f}".format(100 * evaluation["CLAS"].f1))
